...
_RETRY_DELAYS = (5, 10, 15)  # segundos entre reintentos
    ...
    for attempt, delay in enumerate((*_RETRY_DELAYS, None), start=1):
        try:
            ...
            if hist.empty or len(hist) < 21:
                raise ValueError(f"historial insuficiente ({len(hist)} filas)")
            ...
            if delay is not None:
                logger.warning(f"{symbol}: intento {attempt}/4 fallido ({e}) — reintentando en {delay}s")
                time.sleep(delay)
            else:
                logger.error(f"{symbol}: todos los intentos fallaron — {e}")
